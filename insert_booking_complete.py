#!/usr/bin/env python3
"""
Insert complete booking data with all required fields to match SQL Server structure.
This script creates a booking that should appear in your inventory management tool.

NOTE: This script inserts into PostgreSQL for testing, but your tool likely connects
to SQL Server. To make the booking appear in your tool, you'll need to:
1. Update the connection string to point to SQL Server
2. Install pyodbc and SQL Server ODBC drivers
"""
...
def insert_complete_booking():
    ...
    try:
        now = datetime.now()
        print("=== INSERTING COMPLETE BOOKING WITH FULL STRUCTURE ===\n")
        ...
        # 6. Insert equipment items with all required fields
        ...
        # In PostgreSQL, we use the simplified schema but include the product code in the description
        for product_code, description, qty, hire_rate, heading_no, seq_no, sub_seq_no, item_type in equipment_items:
            cursor.execute("""
...
"""
        ...
        # Commit transaction
        conn.commit()
        print(f"\n✅ SUCCESS! Complete booking created: {booking_no}")
...
def main():
    """Main function"""
    print("=== INSERT COMPLETE BOOKING WITH FULL STRUCTURE ===")
    print("This will create a booking with all required fields to match SQL Server structure.")
    print()
    
    success = insert_complete_booking()
    
    if success:
        print("\n🎉 Complete booking inserted successfully!")
        print("Now check your inventory management tool to see if this booking appears.")
        print("\nNOTE: If your tool connects to SQL Server and not PostgreSQL,")
        print("you'll need to update this script to connect to SQL Server instead.")
        print("See the SQL Server connection example in analyze_sql_server_booking.py")
    else:
